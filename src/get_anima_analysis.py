...
def analyze_clouts(data):
  ...
  for idx, tweet in enumerate(data["tweets"]):
    ...
    idx2clout[idx] = simpleSum

  idFreq = sorted(idx2clout.keys(), key = lambda kv: idx2clout[kv])
  mostPopular = idFreq[-1]

  return mostPopular, idFreq

def anima_analyze(data):
  ...
  top3tweets = []
  for idx in idFreq[-3:]:
    top3tweets.append(data["tweets"][idx])
  ...
  # C10:
  final_output["most_popular_tweet"] = data["tweets"][mostPopular]
